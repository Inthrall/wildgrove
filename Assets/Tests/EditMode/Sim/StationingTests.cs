...
namespace Wildgrove.Sim.Tests
{
    ...
    /// <see cref="Stationing.SketchAgentsAt"/>, the watch a site's own post
    /// supplies.
    ///
    ...
    public class StationingTests
    {
        ...
        [Test]
        public void HasBodyAt_ASketchPost_FollowsWhoeverHoldsIt()
        {
            var watch = Familiar.SketchStation("old-growth-wood");
            var warden = new GameState();
            Warden.Sketch(warden, "old-growth-wood");
        ...
        [Test]
        public void HasBodyAt_ASketchingWarden_LeavesEveryNodeAndEveryOtherSiteEmpty()
        {
            ...
            Assert.That(Stationing.HasBodyAt(state, "n1"), Is.False);
            Assert.That(Stationing.HasBodyAt(state, "n2"), Is.False);
            Assert.That(Stationing.HasBodyAt(state, Familiar.SketchStation("old-growth-wood")), Is.True);
            Assert.That(Stationing.HasBodyAt(state, Familiar.SketchStation("mistfen-marsh")), Is.False);
        }
        ...
        [Test]
        public void SketchAgentsAt_ASketchingWarden_DrawsAtTheirOwnSiteAtTheBaseRate()
        {
            ...
            Assert.That(Stationing.SketchAgentsAt(state, null, "old-growth-wood"), Is.EqualTo(1.0).Within(1e-9));
        }

        [Test]
        public void SketchAgentsAt_ASketchingWarden_DrawsAtNoOtherSite()
        {
            ...
            Assert.That(Stationing.SketchAgentsAt(state, null, "mistfen-marsh"), Is.EqualTo(0.0).Within(1e-9));
        }

        [Test]
        public void SketchAgentsAt_NobodySketching_IsNoSketchingAtAll()
        {
            var state = new GameState();
            Warden.Rest(state);

            Assert.That(Stationing.SketchAgentsAt(state, null, "old-growth-wood"), Is.EqualTo(0.0).Within(1e-9));
        }

        [Test]
        public void SketchAgentsAt_AWardenAtANode_DrawsNothing()
        {
        ...
        [Test]
        public void IsSketching_TellsASketchPostFromAGround()
        {
            ...
            // not a ground.
            var warden = new GameState();
            Warden.Sketch(warden, "the-hollows");
            var familiar = WithFamiliarAt(Familiar.SketchStation("mistfen-marsh"));
            var gathering = WithFamiliarAt("n1");

            Assert.That(Warden.IsSketching(new GameState()), Is.False, "an unposted warden");
            Assert.That(Warden.IsSketching(warden), Is.True, "the warden at a site");
            Assert.That(familiar.roster[0].IsSketching, Is.True, "a companion at a site");
            Assert.That(familiar.roster[0].IsSketchingAt("mistfen-marsh"), Is.True, "and it knows which site");
            Assert.That(familiar.roster[0].IsSketchingAt("the-hollows"), Is.False, "but not another's");
            Assert.That(gathering.roster[0].IsSketching, Is.False, "a companion at a node is not watching");
        }
