...
- **The slate's prices read against 120.** The expressive per-run spend (camp 40 + keepsake 20 + queue 20 = ~80) sits against a diligent per-run earn of roughly 70–200 depending on the fold's length, so the full slate is affordable but not automatic on a short run — choices stay real, which is where the prices are meant to sit.
