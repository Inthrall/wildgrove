...
Wildgrove borrows Idle Obelisk's spine (the Trail), its gate (the Rite) and, deliberately, its heart: a small kith of individually upgradeable workers whose builds and postings are the run's central decisions.
...
> kept level. Tools, mastery, richness and slots all multiplied gathering while
> the trail sat flat between rungs, so any tuning was wrong for either a 4- or a
...
1. **Agents at posts.** The warden and each slotted familiar can be **stationed** at a post; a familiar without a post **rests at camp** with no output (a slot is the right to hold a post, §4). Up to six slots by endgame (one to start, per the §4 ladder), one usually on the trail, eight-plus nodes by mid-run: coverage is never enough, and *where the kith stands* is the moment-to-moment decision.
2. **The windfall catch (REWORKED 2026-07-24, replacing tap-to-tend).** A node drifts a **windfall bubble** into the strip now and then; catching it (tap / click / key) is the warden's own act. The catch pockets a flat burst of goods straight to camp, and it **counts as Tending**: the yield burst, the briefly raised Choice chance, and the Rite's tend deeds all ride on the catch. `economy.bubbles` pays a fixed few seconds of a notional gatherer's hands, the same at every node, so opening new ground never shrinks the prize. **Any ground the run can reach drifts one, worked or fallow (DECIDED 2026-08-06, superseding "a fallow node drifts nothing, the bubble is still earned").** The old rule sounded right and played badly: the only nodes staffed are the two or three already drawn on the strip, so every windfall in the game rose from the plates the player was already looking at, and a camp with nobody posted drifted none at all. A windfall is the *land* handing something over, not a wage for standing there, and the reward is metered by the spawn interval and `maxLive` regardless, so widening the pool changes where a windfall comes from without paying the run any faster. Windfalls from unseen ground blow in along the foot of the band rather than rising off a plate. Tending no longer stations the warden (superseding 2026-07-17): standing somewhere is an explicit assignment, one body per post. Attention is rewarded, never demanded.
...
> **Design tension to preserve:** gather rate (stationing + richness) vs. craft rate (queues + materials) stays slightly out of balance, so upgrading one always exposes the next bottleneck. And the craft split is a **four-way dilemma**: craft for the kit, for the caravan, for the spirits, or for the land itself? Tuning rule: no lane may starve.
...
- **Assigned or sketching (DECIDED 2026-07-18; revised 2026-08-09, the post gathers nothing; revised again 2026-08-12, it is a PLACE; renamed 2026-08-13 from *the watch* to *the sketching*).** An assigned agent works its post deliberately; a familiar without a post rests at camp and works nothing (a slot is the right to hold a post, §4, and resting help for free would make the ladder worthless). **Every observation site is its own post** (`sketch:{zone}`), and its holder, warden or familiar, draws at THAT site (§6) and **gathers nothing**, anywhere. The old gather-share (an even split of one gatherer across every node) is retired: a roamer who also gathered was two jobs on one post, and players couldn't say what the post was for. An unassigned warden simply stands at camp (or wherever they last tended), gathering nothing until posted again. *Two consequences worth naming. Unworked ground now earns literally nothing, so a freshly opened zone is silent until a body walks it or a windfall is caught there (watch the first gift pile in the pacing sitting, since a pile is priced in the node's own resource). And drawing the whole map is now a real decision about slots rather than a free consequence of one posting.*
  - **Why the single roaming post went.** One body held "the watch" and sketched at every site on the map at once, so *assign a sketcher here* silently meant *everywhere*: plates filled in at places the player had sent nobody, opening a zone quickened a site nobody was standing at, and there was no way to ask the page why. A post whose reach is one place can be read off the page. The cost is the honest one: covering four sites now costs four slots, the same shape as every other post in the game. *Save v53 carries the retired post's holder to the first open site's post rather than resting them (`SaveCodec.RestoreStation`), so nobody loads to find their sketcher at camp.*
  - **Why it stopped being called the watch (2026-08-13, Mo).** *Watching* named the waiting and not the work, so the post read as a body standing idle at a place, and its id (`dig:{zone}`) still carried the excavation vocabulary that observe-sketch-release replaced two renames ago. *Sketching* is what actually happens there, it is the word §6 already uses for the output, and the post now wears a plate of its own on the world strip rather than being the one post a player could fill and never see (`NodeWorldView.CreateSketching`). **Save v54 rewrites the ids** in place, carrying the zone half through untouched, so no content data is consulted and nobody's sketcher comes home. The remaining half of the rename is data, not code: `economy.observation.watchXpPerHour`, `pityTimerHoursWatched` and the `digSite`/`digSpeedMult` family still speak the old vocabulary, and moving those is a schema change with a `GameData.asset` re-import behind it (todo §2).
...
- **Reachability.** Anywhere the design says "reachable", most importantly in the Rite validator (§8), it means *satisfiable under plausible stationing with the current kith size*, not merely unlocked.
...
| 6 | **The Hollows**       | Deep ores, crystals, ashglass         | Delving; the rarest insects; **the deep amber** | Echo Geode          | v1.1 (**built 2026-07-29**, and bone beds stopped being a crop for the reason fireflies did: the buried past is watched, never gathered (§6); the third find is ashglass, the glass the burning left) |
...
There is no carrier type, and since 2026-07-31 no carrying at all: deliveries are automatic. A slot is *the right to hold a post*: companions past the slots stay in the kith but **rest at camp** with no post, no output and no run XP. Slots never cap the collection, only who's out working.

| Slot        | When                        | How                                                                    |
| ----------- | --------------------------- | ----------------------------------------------------------------------- |
| 1           | Free from minute one        | The land's first gesture: a vole and a raven arrive unasked; one takes the single post, the other rests, and the first stationing swap is the tutorial. |
| 2–4         | Early / mid / late          | **The first time each named verse is sung** (`economy.kith.slotVerseZones`: the hedgerows, the marsh, the peaks; a verse sung is never unsung, and the record crosses Migrations). **DECIDED 2026-08-11 (Mo), replacing a lifetime TALLY** of 2 / 5 / 10 verses sung: a tally paid for folding early and often, which is the opposite of what a place at the warden's side should mark, and it is un-signpostable, since “sing three more verses” tells a player to grind where “sing the marsh's verse” tells them where to walk. The three are picked off the zones' own `minMigration` gates so they land early / mid / late: bramble-hedgerows is the second trail (mid-run-1), mistfen-marsh opens at fold 2 (run 3), cloudreach-peaks at fold 5 (run 6). Sunfield's verse is left out deliberately, since it is the first thing anyone sings, before a place means anything, and so is old-growth-wood's, because run 1's last verse is the fold gate and a place there would pay for finishing rather than for going deeper. The last rung sits on the last trail (Mo's call, 2026-08-11): the peaks therefore carry both it and the run's narrative capstone, and the crags are where it moves back to if that reads as one beat too many at the same fire. |
...
**The calling gift (DECIDED 2026-08-06):** answering a pile also asks a little Amber (`economy.amber.callingGiftAmber`, first guess 10, §9's early-game sink). The pile stays the gate; the gift prices the *yes*, never the right to ask. Guard rails: **unasked arrivals never ask**, meaning the first vole and raven, a bonded companion crossing the fold, and the Drover's pony; and a short warden's pile **waits, unconsumed**, until the gift can be met, never refused for want of Amber. Priced inside a week's free earn so it is a save-up, never a paywall.

**Mini-wardens.** A familiar is stationed exactly as the warden is: one agent system (§2), and the trail is simply another post. Assign it to a node and it works there, a steady gather trickle plus a slow tending cadence if its build allows; assign it to the trail and it carries. Stationing scarce agents across abundant nodes is the Obelisk allocation decision.

**Traits**, one per species, fixed for life (reworked 2026-07-23; replaces the level-5 powerup picks):

- Each species carries a **single fixed trait**, which is what makes it the specialist of one post. A familiar *comes with* its ability; there is no per-familiar build, no respec because there is nothing to respec. Species is identity: unique plate art, one trait, and **at most one familiar of each species, ever**.
- A familiar still earns **XP at its post**, from its own work. **Levels never scale output**: they pace XP and Kinship only, and yield comes from tools (the post's levers), the species trait (the holder's), and richness.
- Trait kinds the sim reads: `nodeYieldBonus` (with a **pair** of related resources, so the specialist works either node), `trailThroughputBonus`, `digSpeedBonus` (the watch), `choiceBonus`.
- **A node specialist works a related pair of nodes** (reworked 2026-07-24), so one familiar answers either node's gift pile and boosts both wherever it's posted (a watcher gathers nothing, 2026-08-09). The starting kith is sized to the gatherable nodes, seven pair-specialists covering zones 1–4 (plus rare-herbs), and each species' animal is chosen to match the powerup it gives. Names each start with a distinct letter.
...
- **Kinship (never resets):** each roster familiar carries a permanent **Kinship level**: *the creature's memory of careful hands*, as Verdure is the land's. At Migration, a familiar's run XP converts to Kinship XP (conversion only, since run XP already credited Renown as it was earned; **no second Renown grant**). The √ conversion decelerates in parallel with Verdure, both permanent tracks flattening together, and Kinship gains appear **in the same fold forecast panel** (§8), so the creature's memory never quietly argues against leaving. Kinship gives small permanent perks: **higher starting level** and **+XP rate**. At Kinship milestones a familiar's **signature deepens** (**built 2026-07-28** as trait deepening, since the species trait is already its identity, so milestones sharpen it rather than add a second system): at Kinship 2 / 4 / 7 (`familiarXp.signatureMilestones`) the trait's value scales by +25% per milestone (`signatureDeepening`), and the plate takes an authored **inscription line** in the warden's hand (§7's channel about individuals; `species.json inscriptions`). The fold forecast names a sharpening before it lands, so the creature's memory argues *for* the fold in its own voice.
- *(Vocabulary: the permanent track is **Kinship**, never "bond level", since "bond" belongs to Migration-crossing, below.)*

**Bonding** is the separate, rarer honour (earned, never bought): a **bonded** familiar crosses the fold and is present from minute one, and its Kinship is why it is also *good* from minute one. Most of the roster slips back into the grass at Migration and is re-met in later regions, a quiet reunion beat rather than a re-grind: roster and Kinship persist, only presence lapses. MVP: 1–2 bondable (final counts and rarity: Mo to settle, §14).

**Deliberately not adopted:** Almanac-mediated familiar power ("familiars start at level 5" Verdure nodes). Familiar permanence is exclusively Kinship: the relationship is with the creature, not purchased from the tree. The Almanac's familiar-adjacent nodes are limited to *The Old Friend* (the bond alone, since slots come from verses and the store now) and *The First Planting*.

**Roosts & Burrows:** the building line levels **familiar comfort**: +XP rate for all stationed familiars per level, and late levels add roster capacity. Depth, not headcount.
...
| Birch Frame Pack | Pack  | Bushcraft | Timber ×25, cordage ×5 | The warden's own hands +20% (hauling retired, so the pack quickens the warden, not a lane) |
...
- **Amber is the exception, and stays takeable:** old resin with an ancient insect sealed inside, the one creature the land lets you keep, because it was gone long before you came. It surfaces at the watch alongside the sketching and outlasts it, since a fully-recorded map keeps turning up resin, and it is the only window the ground still opens onto the deep past. What that watch is *worth* in amber no longer scales with how much ground is open or how good the screens are (§9).
...
| **The Lantern Bearers**  | 4        | Mistfen Marsh          | +20% observation speed     | They light the drowned paths every night, for no one. Fireflies are **watched, never gathered**, so the marsh's third *find* is glow-moss (corrected 2026-07-28; a firefly in a basket contradicted §6 outright). |
...
- Each new system's *first* margin note doubles as its instruction, in voice ("left a pile by the bramble; something is watching it"), so the teaching pass is a writing task, not a tutorial system.
...
- **A sung verse opens the next zone's gathering (BUILT 2026-08-05).** The Rite is not a tax levied beside the trail, it is the trail: answer the meadow and the hedgerows open, answer the hedgerows and the wood does. This closes the loop the verse order already implied, since each verse is sung in the ground the one before it earned, and it makes the run's spine a single chain the player can name, rather than a ladder purchase running in parallel with a ritual. The zone's fold gate and tool tier still hold (§3, §8): the verse says *when you have earned it*, those two say *when you can survive it*, and a trail waiting on either arrives the moment it is answered.
...
- **A verse asks for the country it just opened (2026-07-31).** The generator's picks lean to the goods that debuted latest, reaching back to older ones only when the new tier runs out. The candidate pool is unchanged, since this only reorders it, but without the lean a Crags verse could ask for berry preserves the camp has stockpiled since the first hour, and a verse that is already answered when it appears is not a gate.
...
- **Offerings are whole (BUILT 2026-07-31).** A slot takes its entire ask in one act or not at all: the button stays shut until the stores hold all of it, and the row reads **have / asked** so the gap is the thing you watch. Holding part of an ask is not progress, and no part-answer is banked for the verses behind it to inherit: what a verse asks, it asks in full. *Why:* dribbling stock into five slots made the size of an ask meaningless, since a slot filled a grain at a time costs only patience; the decision the Rite is meant to pose is which whole ask this run can actually reach. Offerings are consumed on delivery and **credit Renown at full trade value** (§9): you give up liquidity, never prestige progress. Deed, specimen, and field-sketch slots carry fixed Renown grants, and the sketch's is the largest, as the steepest thing a warden can give (§6). Gifts and replanting earn no Renown: feeding the spirits is remembered; feeding the voles is lunch.
...
- **Bonded familiars** cross the fold, present and Kinship-strong from minute one, which is much of why run 2 feels faster.
- **Region modifiers, RETIRED (DECIDED 2026-08-08, superseding 2026-07-28).** The per-fold drawn season (lush / misted / ashen / windswept, `regions.json`) is replaced by **the Wheel (§15)**: the world's lean now comes from the real calendar's tide, not a per-run draw, so there is one season system rather than two, and it is the one that keeps a real calendar. What the draw used to do passes over or lapses: the fold forecast named **the next sabbat** instead of the region ahead (**cut 2026-09-15** — the Wheel turns whether or not the camp folds, so the season was never part of this choice, and the events rail reads it on every tab; the confirm keeps what is gained, what is lost and the two ways out, and nothing else); the vignette keeps its twelve words without the season's sign; the Rite generator prices from migration count × unlocked content alone (`modifierWeight` and `regionSeed` retire with the draw); run 1 no longer needs a home-ground guarantee, since every run is home ground now; and fielding reads the tide instead (§15, where Beltane is the vole's fortnight). One honest loss: folding no longer changes the world's flavour, so "a better season ahead" stops being an argument for the fold, and the forecast's case rests on the permanent tracks alone. **The per-run randomness is parked, not condemned** (todo Appendix A): if the Wheel alone leaves runs too samey, a drawn overlay can return, but as the *second* flavour, judged beside the one that keeps the calendar.
...
The pacing knobs (`demandGrowth`, the breadth ramp) only change *how fast* the same six zones go by; they cannot stop a run from reaching the end of the map. The fold gate is the lever that can. A zone may carry a **`minMigration`**, a count of folds that must be behind the warden before that trail exists at all, so the far country is something the warden earns runs to see rather than a wall to grind through on the first.
...
- **Known consequence:** fewer verses early means fewer gift piles. It also used to mean a slower kith ladder, with the second slot slipping to run 2, which is one of the two things that retired the lifetime tally on 2026-08-11 (§4): the ladder now hangs off named verses, and bramble-hedgerows was chosen for the first of them precisely to put that place back in run 1. The gift piles still read off the tally and still thin out early; watch those in a real run-1-to-run-5 sitting.
...
- **Granted rungs** (`grantUpgrade`, Almanac-only): an owned node puts a named ladder rung on every run **free**, with no materials and no skill gate, and the grant is the head start. *The Remembered Edge I/II* start the run at flint then copper tools; *The Known Way I/II* start it with the Bramble then Old-Growth trail maps, one requires chain (edge→way→edge→way) so the tool always precedes the trail that demands it. The **fold gate and the tool requirement still hold**, so a granted trail behind `minMigration` arrives on the fold that earns it, and the validator refuses a map grant whose requires chain doesn't carry the zone's covering tool (a bought node that does nothing is worse than a refused one). Recruit rungs can never be granted: familiar permanence is Kinship's alone (§4).
...
Costs (6/10/14/22 up the granted chain, 8 for the Fire Remembers; the one-off tree now totals 159) are first guesses tuned so the line opens around folds 2–4, and the same playtest sitting as the rest of the pacing pass should set them.
...
The √ curves do the design work: each Verdure point costs more Renown than the last, each Kinship level more XP, and **both permanent tracks flatten together**, so the forecast visibly decelerates while the fresh-run alternative (fast early points, compounding Almanac bonuses) grows relatively better the longer you linger (the previewed modifier left with the drawn season, 2026-08-08; the Wheel turns whether or not the camp folds). Renown lands in **chunks** as well as trickle, since verse completions, familiar level-ups, Folio fixings and sketched portions carry fixed grants, so "stay for one more level-up" gives the curve texture. **Guard rail: the forecast sets timing only.** The Rite remains the sole gate; no minimum-Verdure or minimum-Kinship requirement, ever.
...
| Camp buildings        | **Material bundles**, and Bushcraft is the construction backbone                     |
| Trail Maps            | **Provisions**: a goods bundle; you pack for the walk, you don't buy it              |
| Selling               | **The Exchange**: the caravan barters goods for goods. Rates are **always derived from the single trade-value table** (never authored per pair, since hand-set pairs breed arbitrage), less a spread; small trades **round in the player's favour**, because the caravan is dry, not petty. Off-zone inputs (berries for nuts) are its real job; trade goods remain the densest barter weight. A pressed **consideration** in Amber re-deals the standing offer at once (§9 sinks, 2026-08-06). |

**The pacing spine is XP**, on two tracks: warden craft XP (nine skills, gating tools and recipes) and familiar XP (gating powerups; earned at the post, so *where the kith stands is how the run is spent*).
...
| **Amber**   | Hard / premium   | IAP, observation sites, rewarded ads, weekly Play Games Reward  | Time-skips (paid skips budgeted to `timeSkipDailyCapHours`/day, where 24 pins a heavy spender to ≤×2 a free player's pace, since sim-time is the only thing money buys; **built 2026-07-30**) · namings (companion 30 / warden 50, **built 2026-08-05**) · the 2026-08-06 slate (**built end-to-end 2026-08-06**, with a click-through and wording pass owed, todo §1.5): camp name · keepsake page · second queue · drover's consideration · settle-the-ledger (through the skip budget) · the calling gift (§4). Cosmetics stay retired for want of a substrate |
...
- **The second queue** *(convenience · 40 · per run)*, one extra craft-queue slot at the stations, lapsing at the fold so it recurs. It removes craft *latency*, never material walls, since a queue cannot mint inputs, and it stays at **+1**: this is the one convenience that adds throughput outside the skip budget, and breadth here would be pace by another name. **Raised 20 → 40 on 2026-08-13**, when the camp's name went permanent and left the queue as the slate's only *guaranteed* recurring purchase: it takes over the 40/fold the name used to ask, in the lane that is designed to lapse. It was underpriced at 20 regardless: 40 is 2.7 skips, about 10 sim-hours, bought against a second standing order at every station for a run of four to twelve days.
- **A consideration for the drover** *(convenience · 5 · repeatable)*: press a little Amber on the caravan and the standing deal **re-draws now**, excluding the deal it replaces so the coin always changes something. Mechanically: a per-window re-deal count persists in the save and mixes into the window's seed, so the reload-never-rerolls invariant holds as (window, considerations) → deal, and the count resets when the window turns. *Rating note (checked 2026-08-06):* bribing a fictional trader is not an IARC/PEGI/ESRB questionnaire item; the line to stay clear of is Play's loot-box disclosure rule, and a re-dealt *offer* grants no item, since the purchase changes what the caravan asks rather than what the player receives, with the draw uniform over discovered goods if disclosure is ever wanted anyway. "Bribe" is the design word, not the page's: the caravan is dry, not petty, and the in-game wording is the narrative pass's, in register (*a consideration*, *sweeten the deal*).
- **Settle the ledger** *(pace · through the budget)*: when an absence outran the offline cap, the welcome-back sheet offers to credit the **uncovered hours** of gathering and production at the **full live rate**, priced pro-rata on the skip (15 per 4 h ≈ 3.75/h) and **drawn from the same leaky skip budget**, so the ×2 pin holds unchanged and no new throttle rule exists. The moment of loss on return is the game's best-converting placement, which is why doubled offline earnings is the best ad slot, and this ships on plumbing that already exists (`SkipBudgetHours`).
...
- **The slate's prices read against 120.** *(Re-counted 2026-08-13, when the camp's name was made permanent and the queue was raised to cover it.)* Run 1 spends **80** (camp 40 + queue 40); **every fold after it spends 40**, the queue alone, against a diligent per-run earn of roughly 70–200 depending on the fold's length. The recurring number is back where the slate was balanced; it just moved out of the expressive lane and into the convenience one, which is the lane that can honestly lapse. Choices stay real on a short fold and the guaranteed save-target survives, which is what the drip is claimed *for*.

- **What holds the drip is the skip, not the slate (DECIDED 2026-08-13).** The re-count above was nearly a false alarm, and the reasoning is worth keeping. The paid skip is an **unbounded** sink at the free rate: `timeSkipDailyCapHours` 24 allows six skips a day (90 Amber), against a diligent earn of ~17.7 a day, so the budget can never bind on a free player and their pile cannot fill while they want sim-time at all. The "pile with nothing to want" the slate was built for therefore belongs entirely to the player who does *not* skip, and for them the expressive lane is the whole of demand, which is exactly what the camp's name was carrying. What still recurs there is a **companion's naming at 30**, because arrivals keep coming and only a *changed* name is charged for. That is the last recurring expressive sink, so it is the one price on the slate that should NOT rise: making it dear enough to skip removes the demand rather than deepening it. Nor should the **skip (15)** or the **consideration (5)**: the skip is the free player's only pace lever and its demand is already infinite, and the consideration's cheapness is what makes it repeat, a two-drip loop being the best drip demand on the slate.
- **The built prices (skip 15, namings 30/50) now read cheap against the anchor.** Left deliberately, since generosity is the posture, but they are the first candidates to raise if the diligent player's pile still grows unchecked in playtest.
- The economy.json `$note`s still cite the ~40/week lean; refresh them when the slate's keys land.

**Considered and rejected (2026-08-06), so the boundary is written down:** a Rite slot re-roll or offering discount (sells the Rite's decision, and "never sold" is spirit, not letter) · Amber→gifts (buys a creature with one hop of indirection; the calling gift deliberately prices the *answer*, never adds a pile) · choosing the next region's season (breaks the deterministic draw; sells run flavour, moot since the drawn season retired, 2026-08-08 §8, but the boundary stands: run flavour is never sold) · any Amber→Verdure/Renown bridge (sells score and permanence; the money-clean XP rule exists for this).
...
- **Gates** (the Rite) may grow at most linearly. `d` sits just above the settled power ratio; the real growth is `verseSlots(m)`, which is **breadth**: it costs stationing, kith slots and map coverage rather than wait-time, and it is hard-bounded by how many slots a verse has. The generator widens each verse in step so the choice margin never narrows, and a per-verse clamp guarantees at least one slot of choice even where a lean zone can't be widened.
...
| 3  | Drying Rack                  | Camp       | none            | Fibres ×80, wildflowers ×30          | Berry barter weight +25%                             |
...
| 7  | Camp Fire Ring               | Camp       | none            | Stone ×40, fibres ×100               | Unlock Firecraft + Forgecraft (copper on open fire) + Preserve recipe |
| 8  | Copper Sickle                | Tools      | Foraging 12     | Copper ingots ×5                     | Foraging yield ×2                                    |
| 9  | Root Cellar                  | Camp       | none            | Stone ×80, timber ×20 (Exchange)     | Offline cap 2 h → 3 h                                |
...
| 14 | Carving Bench                | Camp       | none            | Timber ×60, stone ×30                | Unlock Bushcraft + Plank, Cordage & **Planter** recipes |
...
| 20 | Smokehouse                   | Camp       | none            | Planks ×50, clay ×120, stone ×60     | Offline cap 3 h → 5 h                                |
...
| 29 | Almanac Desk                 | Camp       | none            | Planks ×80, iron fittings ×10        | The fold forecast (§8 decision surface)              |
...
**Rewarded video:** ×2 yields and XP rates for 4 min (stackable to 1 h) · double offline earnings on return (the single highest-value placement) · instant-finish a craft queue · small Amber drip. *(Time-boxed boosts may touch XP; permanent multipliers never do; see the next line.)*

**IAP:** **Warden's Sigil**, a permanent **+20% to yields and craft speed** (~US$7); **permanent XP multipliers are never sold**, so familiar growth, Kinship, and Renown pacing stay money-clean · remove ads · Amber packs · starter bundle · cosmetic camp, gear & familiar skins (skins never touch Kinship or builds).

**DECIDED 2026-08-01: the Sigil shrank from ×2 (Mo's call).** Sim-time is the only thing money buys here, and §10's paid-skip budget already caps that at ×2 a free player's day. A permanent ×2 on yields *and* craft speed stacked on top of that ceiling forever: it halved the whole map's walk (~9–12 FTP days to ~2–3) and turned one purchase into a second pace. At +20% the Sigil takes a sixth or so off a run and changes no wall's character: the craft-XP gates (firecraft 28, forgecraft 40), the fold gate and the skip budget stay exactly where they were placed. It is a thank-you that tilts the grove, not a tier of the game. If it ever needs to feel bigger, the lever is **breadth**, another cosmetic line or more Amber in the bundle, not a fatter multiplier.

**Never sold: the Rite. Never sold: a creature or a Kinship level.** Amber accelerates production; it cannot fill a verse slot, level a familiar, or add a gift pile: the right to call a creature is earned per verse, only. *(Amended 2026-08-06: answering a pile now asks a small Amber **calling gift** (§4, §9), priced inside a week's free earn so it is a save-up, never a paywall; the earned pile remains the gate money cannot touch.)* (The last two *slots* are sold: the right to field more, never the friends themselves.)

**Play Games Rewards (Level Up requirement, not IAP):** **The Drover's Halter** (a fell pony that carries for the warden, so their own gathering +20%) + **The Wayfarer's Plate** (a plate drawn by another hand, arriving already recorded) single-use by Sep 30 2026 · Weekly Amber Cache (20, max 1/wk) repeatable by Mar 1 2027.

**BUILT 2026-07-29: how a reward reaches the grove.** A reward is an ordinary one-time product with a Play Games Reward offer attached: Google awards it for a Quest (single-use) or a Social Challenge (repeatable) and delivers it through the **out-of-app purchase flow**, so the game receives it like any other purchase. The order the game owes the player is **grant → tell them → acknowledge**, which makes refusing to acknowledge the *safe* failure and acknowledging-before-granting the unsafe one. *(Corrected 2026-08-12: the three days are real but this used to describe them wrongly. Google's Rewards page says the **player** has up to three days to open the game and claim, after which the reward is no longer available; it does not say an unacknowledged order is refunded and re-offered. The conclusion survives the correction, because a reward we acknowledge without granting is lost either way, but the same page adds a step we do not fully take: it asks the game to check for unacknowledged rewards when it **starts or is foregrounded**; see todo §3.4.)* **The debt is persisted, from 2026-09-10 (save rung 55).** The grant banks its credit the moment Play hands the reward over and the sheet can only be shown by the HUD, so a process that died in between used to leave the player paid and never told, permanently: the queue holding the confirmation was in memory alone. The run now writes down which reward ids it still owes a telling for (`GameState.rewardsOwedTelling`), the words are re-read from code rather than persisted beside the debt (so a telling that waits a build over reads in that build's words, and no save carries prose it can never be rid of), and the debt is struck off by the player's own acknowledgement rather than by the sheet opening. A reward is therefore told twice at worst, which is the harmless way round, and never lost. The debt crosses a fold and follows the book rather than the device when a cloud save is adopted. The confirmation is a compliance artifact and outranks the journal's usual reticence: the item is named plainly, Play Games is said out loud, there is no way to decline, and it stays up until the player acknowledges it. All three land through this. **The cosmetic cloak that used to stand here was retired unbuilt (2026-07-30):** it needed a cosmetic substrate the game has never had, and building one to justify a single reward is the tail wagging the dog. **The Wayfarer's Plate** replaced it and meets the Sep 30 2026 bar of two single-use items. The plate is the one page no observation site can offer: it arrives already recorded, drawn by a hand that walked here first, which is also the only shape that makes an out-of-app arrival diegetic, the game's whole narrative register being marks left by others. It is deliberately the **weakest plate in the Folio**, because its value is that no one walked for it. It needs no Migration handling of its own, since recorded plates already cross the fold, so writing it into the Folio rather than deriving it from the entitlement is what makes it permanent for free. The wayfarer survives as the figure who drew it.
...
- **holds no slot.** The lane costs nothing from the §4 ladder, so the reward lands the moment it is redeemed rather than waiting for a slot the player may not have. Because the pony can stand nowhere else, the exemption cannot leak into gathering, which is the reason it is an animal and not an abstract free post.
...
| Achievements (10 min / 40+ rec / 4 in first hour ~~due July 2026~~) | 40+ from zones, crafts, spreads, plates, verses, Rites, Migrations, Kinship milestones; first-hour four per §10. **Re-read 2026-08-12: the thresholds hold and the July 2026 due date does not appear on the guideline at all**, so treat it as unsourced rather than met. The first-hour four is specifically the **Quest eligibility** bar, which makes it load-bearing for the Rewards row two lines down: single-use rewards are awarded on Quest completion, so no Quest-eligible achievement set means no way to deliver them. **45 published**, and `AchievementIds.g.cs` is generated by `pgs-achievements.py` from the console round-trip. **⚠️ A published incremental achievement's step count CANNOT be edited — found in the console 2026-09-09, where the field is greyed out over "This can't be changed after the achievement is published", and there is no Delete either.** So the three drifted counts are frozen at **12, 5 and 14** against data holding 14, 7 and 19, and all three fire early against their own names. **DECIDED (Mo): reword rather than add a completion tier.** In-repo the same day — targets reverted to the frozen figures, tests re-pointed to guard *against* anyone raising them again, and the manifest renamed to Twelve Companions / Five Plates Drawn / The Almanac Well Thumbed. Owed in the console: those three renames plus the Pristine→Choice one and Fixed in Ink's description (todo §2). **The lesson worth carrying: get an incremental achievement's step count right before publishing it, because that is the only chance you get.** | 5     |
| Game Stats (≥5 repetitive, ≥1 competitive, ≥1 progression)    | **Wired 2026-07-29** (`GameStats`): resources gathered (SUM, competitive) · goods crafted (SUM) · windfalls caught · specimens fixed · verses sung · Migrations (COUNT) · **trails walked** as the progression level. Continuous stats post as deltas against a baseline; cadence is the save cadence. **Submitting since 2026-08-04** on GPGS 2.2.0 (`RecordEvent` + `RequestEventsUpload`). **Uploaded and accepted 2026-08-13**: all seven events and seven stats sit in the console as Draft, available to testers. **DECIDED 2026-09-09 (Mo): publish it, taking the config to production alongside §2's corrected achievement step counts** — publish is project-wide, so it is one act; closed testing had not begun, so there were no testers or players for a production config to disturb, which made it the cheapest that click would ever be. **Re-read 2026-09-09: the guide has been rewritten and the caveats are gone** — no "beta", no "early feedback", no undated "GA starting August 2026"; one milestone row remains, September 2026 for players seeing stats on the Gamer profile. GA has happened. Owed: a tester read of the draft. Schema authored in `store/play-games/gamestats/` | 5     |
...
| Rewards items (2 single-use by Sep 30 2026 / 1 repeatable by Mar 1 2027) | Drover's Halter, Wayfarer's Plate, Weekly Amber Cache (the cloak was retired unbuilt 2026-07-30, §11). **Delivery path built 2026-07-29**, using the out-of-app purchase flow, grant → tell → acknowledge, and all three products live in the console (2026-08-04). **✅ All three offers created and ACTIVATED in the console 2026-09-09, so `LU-RE-GAC` is met end to end** and `LU-RE-GAD` is met five months before it is due; the remaining unknown is a real delivery on a device (todo §3.3). The bar was already met in code and catalogue. **The association UI opened Sep 1 2026 as promised** (re-verified 2026-09-09: the Rewards page still reads "available from September 01, 2026" for adding, removing and proving delivery, and nothing was pushed back), so attaching the offers is now the live task and `LU-RE-GAC`'s Sep 30 window is the one actually running. **Level Up enrollment also opened Sep 1 2026** (Play Console → Monitor and improve → Policy and programs → Programs → Level Up), but it is per-title and requires the game to be **published to production**, so it belongs to the launch tail rather than to this visit; Sep 30 is therefore a guideline phase-in for us, not a deadline we can miss (todo §3.5) | 6     |
...
| Full keyboard/mouse + controller                              | Input abstraction from Phase 1; **journal navigation built 2026-07-29**: arrows/d-pad/stick move an ochre focus mark, Submit presses it, B/Esc backs out, the shoulders (or Q/E) turn the page, pad-X/C catches a windfall whatever is focused. **Declared 2026-08-01**: the build writes `android.hardware.gamepad` and `android.hardware.touchscreen` into the manifest as not-required, since Play reads the manifest rather than the build, and an undeclared touchscreen is *assumed* required. A lit rename field now owns the keyboard, so nothing typed navigates. Remaining: play it through on real hardware with a pad | 2     |
...
- Data-driven content: resources/upgrades/recipes/gear/planters/species-pools/rites/dialogue as JSON in `design/data/`, validated into a ScriptableObject database at editor-load/build, so balancing and writing never require code changes
...
- Offline progress (2 h cap at the start, 12 h fully walked out, per the stationing rules in §2) + a welcome-back summary of the absence's harvest
...
**Gate (two questions):** *is 20 minutes fun?* (hand it to 3–5 people and watch where they stall) and *can a new player say what anything is worth without Coin?* If either fails with placeholder art, stop and fix; content won't save it.

### Phase 2: Adaptive UI & input (2–3 wks)

- The five-page journal: **Trail** (zones, stationing, replanting, verse sites; the map is the page's own navigation; each ground folds shut behind its name and only the newest stands open, so eight zones of plates stay one readable page) · **Camp** (queues, buildings) · **Stores** (the drawer, the tinctures and the Exchange; see below) · **Warden** (kit, skills, roster & slots, stats; the Almanac appears here after the first Migration) · **Record** (Compendium, Folio, Deep Pages). The Rite has **no tab**: verses live at their sites, with the compact tracker pinned on every page. *(The map is never called "Almanac.")*
- **The Stores drawer** (DECIDED 2026-08-04): what the camp holds *now*, as a grid of square plates rather than a list of names and numerals, since the game's collectibles are all plates (§6), and the one page about owning them was the one page that never showed one. **One tile per stack, and the border is the grade:** the loot scale players already know, white for plain, green for Decent, blue for Choice, at printed-pigment saturation (`#FFFFFF` / `#338C21` / `#1A6BB8`). Took three passes, DECIDED 2026-08-04: parchment-mixed hues read as one border in three lights, WoW verbatim glowed like a backlight on the paper; the keeper is WoW's hues at the journal's ink weight. The three held figures moved off the Compendium, which had been answering *what have I ever found* and *what can I spend* in the same line and neither legibly; the Compendium keeps the lifetime record. **The tinctures moved here too**, a bottle being stock, the one thing on the Warden page that could be counted, spent and run out of, and they are drunk by tapping the tile.
- **The Record page is drawn in plates, and what only records folds** (DECIDED 2026-08-13): the same argument as the Stores drawer above, applied to the page the collection actually lives on. The Compendium and the Crafts are drawers of square plates; a Folio spread is drawn as the strip of specimens it asks for, in full ink where one is pressed and pencilled in where the page is still bare, so the card says *which* specimen it wants rather than only how many; an uncaught insect's sketch ladder is drawn as marks. Two things the page never said, it now says: **what a Folio spread grants** (it asked the warden to consume a Choice find forever, the windfall's third fork in §6, and priced it at a name) and **what a finished insect plate grants** (the largest permanent multipliers in the game were the ones nothing named). And the cards that only *record* (Compendium, Deep Pages, Wheel) fold shut behind their own tally, which is the Trail's rule for its grounds applied to the longer page; the cards you *act on* lead and stay open, the Almanac first (`JournalCardFolds`). Compendium **entry text** is still unwritten and belongs to the narrative pass (todo §1.2).
- **The Trail's head is short, and that is a rule** (DECIDED 2026-08-13, Mo's call): everything above the first ground is status, and the plates are the only thing on the page to act on, so a pinned line at the top of the Trail has to be worth a plate, and almost nothing is. It had grown to the tide's sign, the tide's touch label, an unfolded keeping card and the trail-home bar: roughly 1,480 canvas units of preamble against a ~1,030-unit viewport on a reference phone, which put the first gathering plate a viewport and a half down the one page whose whole subject is gathering, for the ~68% of the year a tide holds (§15's `openDaysBefore` 30). Five cuts, and every one of them is a thing said better somewhere the page doesn't pay for: **the fallow weeks' countdown went** (the events rail's coming-sabbat cell is the same `NumberFormat.Countdown` on every tab, so the line was the cell read aloud, and the invisibility it was added to fix had already been fixed by the thing that replaced it); **the touch label went to the tide's sheet**, which carried the same sentence under *While it holds* already; **the keeping card folds shut** behind a live head reading tier · answered · closes-in, plus a moss clause the moment the stores can meet a slot; it is the one card in the book that folds while carrying buttons, earned because both doors into it (the tracker's tide row, the sheet's own button) open the fold on the way through, so it is never a shut drawer at the end of a link; **the tracker dropped its grey `· Ostara-tide` tail**, which restated, two rows above a rail cell wearing that sabbat's plate, exactly the demotion the rail was built to end; and **the trail-home bar went entirely**, 100 units of pinned height animating deliveries that had become automatic, lossless and untappable, an animation of a thing that cannot go wrong drawn above the plates that can. The keystone mark halved, 120 → 60. **A sixth cut, 2026-09-09:** the tide's sign went too, and with it the last thing on this page above the keeping's own folded head. It was a note in the warden's margin about what day it is, and once the tides ran end to end (§15) "what day it is" became a six-week season, so the same scrawl stood over the plates on every visit for the whole of it; it reads once and on purpose on the tide's own sheet, where the sabbat's lore now stands beside it. The head that remains is one folded card, which was the point. **A seventh cut, 2026-09-10 (Mo's call): the keeping's card went too, to the FOOT of the page**, under every ground and the Rite, so the head is now nothing at all and a gathering plate opens the page. It is the card on this page that loses least by the distance: both doors into it scroll straight to it and open the fold on the way, which is the same argument that earned it the fold in the first place. Its head lost its ink rule in the same pass (`FadeButtonBorder`), because a ground's heading is a bare plate standing in the body and a node's card carries no head, so an ink head inside an ink border was the only box in a box on the Trail; the Record's five folding cards keep theirs, where the doubling is the rule rather than the exception. The carrier is gone for good; whether the **fell pony** gets a walk of her own again is deferred (she still reads on the roster, so it is a question about motion, not about visibility), and if it comes back it belongs in the world strip, where ambient motion already lives and costs the page nothing.
- **The tide's sheet is lore and a door; the keeping's card empties as it is kept** (DECIDED 2026-09-10, Mo's call). Two cuts, one argument: every surface the Wheel owns had been saying the same state over again, and the popup was saying it one tap before the page that answers it. **The tide's sheet keeps four things and loses three**, holding the sabbat's plate, its `lore`, its `sign` and the button through to the keeping. The countdown went because it is the rail cell that opened the sheet read aloud, and the keeping card's own head says it a second time at the other end of that button. The slot list went because it IS the card behind the button, drawn twice a tap apart, against the rule every sheet in `JournalSheets.Events` was built on: a popup points at the page that answers it and never becomes one. And the *While it holds ×1.2* line went because the sabbat's `sign` sits directly above it doing the same work in the warden's hand (§15's **seen as a sign**); its arrival here in 2026-08-13 was the Trail's head shedding it, so the numbers have run out of pages rather than moved again. **The keeping's card, meanwhile, shows the work left rather than the work done.** Once all five slots are set down the rows hide and the card is its tier line alone, turned around to say what the season paid (`+25 amber taken`) in place of what to aim at. Five rows reading *cordage, set down* under a head already reading `5 of 5 set down` was the tally spelled out at five times the height, on the one card in the book that folds to save exactly that. The rows hide in the live pass rather than being skipped at the build, so the last offering clears the card and the next sabbat taking the wheel fills it again, neither needing a rebuild of the page. A part-kept season is untouched: an answered row stands while any slot is still open, because there a green line is progress read against the rows beside it rather than a list of the obvious.
- **The Camp page is three cards shorter, and two of them went to a page that had room** (DECIDED 2026-08-13, Mo's call). Late in a run the page measured around 6,240 canvas units against the same ~1,030 viewport the Trail's head was cut to: a camp-actions strip, the camp's name, three station cards (~2,850 of it, the fire alone ~1,500 for ten recipe rows at 132 apiece), five building lines, three Ladder rungs, the Exchange (~950) and the Amber card (~840). Six screens to answer "what is the fire doing". Three cuts, and each one is a card that was in the wrong place rather than a card too long. **The station cards fold**, one standing open by the positional rule the Trail's grounds use (`CampPage.Crafting`, `JournalCardFolds.Station`): the first station with an order standing at it, or the first of all while the camp is idle. Not *every* station that is working, which late in a run is all three and would shorten nothing; and a shut head carries its own bars ("birch charcoal 43%"), so a folded page still answers whether the charcoal is done. A press on a Craft row pins that station open, because the default follows the work and stopping the last order there would otherwise fold the card away under the finger that stopped it. **The Exchange went to Stores**, where a trade is stock for stock and the drawer it acts on is the page: the tinctures made the same move on 2026-08-04 and for the same reason, and a card belongs on the page whose subject it is. **The rewarded time-skip went to the events rail** as a third cell (§10, below), which is a clock and a fingertip standing in the world band's margin rather than a 380-unit plate carrying a bracketed countdown at the head of one tab. What is left on Camp is what the camp does: its name, its stations, its lines, its ladder, and the amber that hastens all of it.
...
- **The Rite, authored run 1:** verse sites, offering delivery, the pinned tracker; reachability made stationing-aware, so a slot counts only if its good's raw-input footprint fits the kith's gather posts (per-slot; `RiteGenerator.StationingFootprint` ≤ `KithGatherPosts`), enforced in the generator's candidate picks and the runs-2–10 ≥3-reachable proof
...
- Migration flow gated by the completed Rite: the **fold forecast** (Verdure + per-familiar Kinship + next-point ETA + region preview, one panel), the vignette, a deliberate confirm; players fear their first prestige, so sell it hard
...
**Gate:** testers migrate voluntarily; run 2 feels faster, worth it, and asks something different; the kith's return feels like a reunion, not a re-grind; and testers can articulate what staying another hour would have bought, which is the unified forecast doing its job.
...
- **Exchange spread value:** ~15% starting point, big enough to stop arbitrage hoarding and small enough to feel generous.
- **Offline magnitude:** per-agent base rates and caps for a ≤6-agent kith, since a night away must still feel generous.
...
- **Amber earn rate:** lean generous, at **~120/week free for the diligent claimer** (drip ≈84 + dig finds ~20 + cache 20; the anchor, DECIDED 2026-08-06, §9 sinks), floor ~40 for the lazy one.

**Before Phase 4**
- **Kinship K_f:** tune so Kinship rewards seasons, not marathon single runs.
- **Roster pacing:** lean 5–6 members by Migration 3, enough that fielding is a choice and few enough that each plate is an event.
...
- **Generator guardrails:** slot spread, spotlight-vs-unlock order, quantity clamps, final-verse reachability under stationing, and powerup-pool coverage (simplified by deterministic pools), with a spreadsheet proof across runs 2–10 before it ships.
...
The device that keeps the world sealed: **the calendar is the warden's, not the land's.** The warden came from somewhere with a reckoning; the journal names the sabbats in the warden's own hand; no spirit is named, none appears, and the land answers the observance as it answers everything, with signs and never words (§7 holds whole). The land has no calendar; it has weather and memory. §7's proper-noun rule survives intact: the wheel arrives as the one set of proper nouns the *player's* world explains instead.
...
- The cycle is **the Wheel**; the open span is **the tide**, as in *Beltane-tide*, the old word English already uses for a festival's span (Yuletide; *tīd*).
...
- **Window: the tide opens ON the sabbat night, at warden-local midnight, and holds until the next sabbat takes the wheel off it. DECIDED 2026-09-09 (Mo), superseding the month-long tide of 2026-08-11 and the fortnight before it.** They are **pagan seasons** now, not windows on a day: eight of them, back to back, no gap and no lead-in, 38 days long at the tightest (Mabon→Samhain) and 53 at the widest. There is no `openDaysBefore` any more — the nights ARE the calendar, and a season's length is the gap to the next one, so the only lever left is which dates are authored. The two earlier shapes were both the same argument being lost twice: a fortnight meant the Wheel was shut two visits in three, a month meant it was shut one in three, and a shape that is shut *at all* is shut invisibly, because the tracker row, the Trail's tide line and the keeping card all stand down together. Laying them end to end retires the question rather than tuning it, and it is what the real Wheel of the Year is anyway: the sabbats are the joints of the year, not eight festivals with dead air between them. What changes elsewhere: the tide's countdown is named for what ends it (*"Beltane in 15 days"*) rather than for a fire of its own; the validator's overlap rule becomes "no two sabbats on one night in one hemisphere", since seasons that run night to night cannot overlap and the only wrong gap is none; the calendar's last authored night holds for 46 days and then the wheel goes quiet, so the last sabbat is not the one that silently never opens; and **the reckoning's lock moves off "a tide is open" onto "the keeping has been begun"** (below), because the old condition would now be true every day of the year. The keeping's cadence is untouched — eight a year before and after, ~6.5 weeks apart, so the Amber lean of §9 does not move. The knock-on to watch is the ambient touch: the world now leans every day there is, so "leaning" IS the baseline and there is no un-leaned week to read it against. If that flattens the touch into wallpaper, the lever is the touch's SIZE, never the window (a season you cannot reach is not a tuning knob).
...
- Offerings **credit Renown at trade value**, exactly as the Rite's do (§8), since the wheel never taxes prestige either. It is no new faucet: the goods were gathered by play; the credit is the standard one.
- **A fold mid-tide** keeps completed slots (the observance is calendar-keyed, not run-keyed); open slots **redraw** against the new run's content, deterministic from (fold count, festival, year), so reload never rerolls and a fold redraws honestly.
...
- **The ambient touch** rides with the tide; it has its own section below.

### The ambient touch: the tide felt at the nodes (**in, 2026-08-08; the world's only lean, replacing the drawn season**)

The world always leans a little toward whichever sabbat holds the wheel: one authored effect per sabbat, small enough to miss without loss, present enough that the calendar lives in the grove and not only in the margin. Mechanically it is one more term in the §9 stack: a `tideMult` that is a single narrow lean inside the authored calendar and 1.0 outside it, standing where the drawn region season used to stand (§8, retired 2026-08-08) but keyed to the calendar rather than the fold. Since 2026-09-09 the "outside" case is only a cursor the calendar does not reach, so in practice one lean is always live and the eight of them hand over to each other at midnight. The gathering leans deliberately echo the specialists' pairs (§4), so a season is also a fielding question: Beltane is the vole's six weeks.
...
- **It never gates.** Nothing is reachable only inside a tide, no content keys on it, and every lean stays inside one mastery band (~+25%), so a missed sabbat is flavour missed, never power lost. This is the FOMO line, and holding it is what makes the touch safe to ship at all.
...
- **Deterministic by construction.** (ratcheted now, hemisphere) → open tide → authored touch. No draw, nothing persisted, nothing to reroll, so reload-never-rerolls holds without new machinery.
- **Seen as a sign, taught on the tide's own sheet (§7's pattern).** The touched node's plate takes a small mark for the span, and the sabbat's `sign` is the instruction: *"The hedge went white overnight and has stayed that way. Flowers and berries come readier than the walk deserves."* Two lines, and they are that sabbat's share of the word budget, the same lines the Costs section counts rather than additions. **They were margin lines until 2026-09-09** — *"Beltane, by my count. …"*, written for the night a sabbat falls on and read at the head of the Trail — and the seasons running end to end is what retired both halves of that: the date-stamp opener, because a season is not a date, and the standing place, because a line that is never not there is a line the page pays for daily (§13 Phase 2). **The sheet's own numbers went on 2026-09-10 (Mo's call), and the sign is now the whole of the teaching**: the *While it holds ×1.2* line had stood directly under the `sign` ever since the Trail's head shed it in 2026-08-13, which is one lean said twice, once in the voice and once in a table. What a season does is read off the warden's sentence about the hedge and the small mark on the touched node's plate, which is what "seen as a sign" asked for in the first place.

Cost, once the calendar service exists for the observance itself: one multiplier hook in the yield path (plus the two odd ducks, a cost factor for Ostara and a spread ease for Mabon, each one line where those numbers are already computed), eight authored entries in `sabbats.json`, the boundary tests, and the margin lines. The risk is tuning, not plumbing.
...
- **Hemisphere defaults from locale/timezone; a journal setting flips it.** The equator simply chooses; a traveller keeps their own reckoning, because the calendar is the warden's. The setting **locks once the open season's keeping has been begun** — anything set down at the fire — and lifts again when the next sabbat takes the wheel. **Changed 2026-09-09 from "locks while a tide is open"**, which was a live condition when tides had gaps between them and is every day of the year now that they run end to end: a setting that can never be changed is not a setting. The vector is unchanged and the new condition is the tighter fit for it — the opposite sabbat covers the very same weeks, so keeping one and then flipping to keep the other is one span of the year claimed twice, and it is the *setting-down* that makes it so, never the flip. Flipping with nothing given costs and earns nothing. Claims key on (sabbat, year, hemisphere) underneath regardless, so clock or timezone games move hours, never rewards.
- **Every window read goes through `GameLoop.NowUnixMs()`**, so the ratchet stays the only clock (todo Appendix B's standing rule). A wound-forward clock celebrates alone, permanently ahead of the real wheel: spent, not minted. With no server, that plus personal-only stakes *is* the integrity story, one more reason the tiers pay pages, not power.
...
- **~150–200 words** across eight tides, each sabbat's line doubling as its teaching note (§7's pattern; the touch's own line is the same line, not an addition), sized deliberately against the post-MVP word budget. **A second line each, 2026-08-13 (`lore`), taking it to ~300:** what the day IS in the old year the warden keeps, rather than a third telling of the reckoning the Record card and the inside cover already say. **Both now stand on the tide's own sheet, 2026-09-09**, the `lore` above the `sign`: a season names itself for a day, so what the day was is the first thing the sheet owes the reader, and the sheet that used to carry the lore is the one that waits a month for a tide to open — which nothing does any more. Both re-voiced the same day off the *"…, by my count"* opener, which dated a line that now runs for six weeks. Still the warden's voice, still no spirit named, and still nothing the land says (§7).
...
- Does the sabbat verse scale with fold count at all? §9's rule says at most linearly; **flat** is simplest and probably right, since the wheel greets every warden the same.
...
- With the drawn season retired (§8) and the tides laid end to end (2026-09-09), do runs read too alike? There are no fallow weeks left to be samey in, so the question is now the sharper one: eight leans a year, one at a time, is the whole of the world's variety. The parked per-run randomness (todo Appendix A) is the lever if that proves thin, and it returns as a second flavour beside the Wheel, never instead of it.
