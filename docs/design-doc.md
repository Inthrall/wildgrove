...
- **Assigned or wandering (DECIDED 2026-07-18; revised 2026-08-09 — wandering is watch-only).** An assigned agent works its post deliberately; a familiar without a post rests at camp and works nothing (a slot is the right to hold a post, §4 — resting help for free would make the ladder worthless). The **wander post is the watch and only the watch**: its holder — warden or familiar — walks every observation site (§6) and **gathers nothing**. The old gather-share (an even split of one gatherer across every node) is retired: a roamer who also gathered was two jobs on one post, and players couldn't say what the post was for. An unassigned warden simply stands at camp (or wherever they last tended), gathering nothing until posted again. *Two consequences worth naming: a save written before the revision keeps its wanderer exactly where it stood and simply stops gathering on load — no migration, nothing lost but a trickle nobody had chosen — and unworked ground now earns literally nothing, so a freshly opened zone is silent until a body walks it or a windfall is caught there (watch the first gift pile in the pacing sitting, since a pile is priced in the node's own resource).*
...
- **A node specialist works a related pair of nodes** (reworked 2026-07-24), so one familiar answers either node's gift pile and boosts both wherever it's posted (wandering no longer gathers, 2026-08-09). The starting kith is sized to the gatherable nodes — seven pair-specialists cover zones 1–4 (plus rare-herbs) — and each species' animal is chosen to match the powerup it gives. Names each start with a distinct letter.
...
- **The slate's prices read against 120.** The expressive per-run spend (camp 40 + queue 20 = ~60 since the keepsake page was cut) sits against a diligent per-run earn of roughly 70–200 depending on the fold's length, so the slate is affordable but not automatic on a short run — choices stay real, which is where the prices are meant to sit. The cut took 20 off the per-run spend, so the recurring slate now clears on a short fold; if playtest shows the pile growing unchecked, this is the first place a price should rise rather than a new sink appear.
...
offlineEarn       = Σ per node: gather rate · min(t, cap)   the trail's own rate and its unheld ×0.5 lane retired with hauling (2026-07-31); wanderers gather nothing (2026-08-09)
...
- **The events rail** *(added 2026-08-11)* — a column of cells down the **left edge of the world strip**, each a face, a countdown and a tap into its own popup. It exists because the Wheel's five surfaces were all conditional and all elsewhere: four of them draw nothing outside a tide, and inside one the tide could still be demoted to a grey suffix by any pinned verse, so the system was invisible to a player who never opened the fold sheet. The rail is the one surface that is always there. Rules it keeps: it costs the STRIP's width, never the page's height (the chrome budget rule, §13 — a bar that earns a pinned line must be read on every tab, and this is read twice a session); cells are a full fingertip or they are dropped, urgent first; a cell never promises what cannot be collected (the weekly cache says *look*, not *take*, and says *sign in* when Play does not know the player); and the sabbat plate appears only once the tide opens, because the plate is what a keeping earns. Its second inhabitant is the **weekly Play cache** (§11) — deliberately, so it is a rail rather than a Wheel widget with delusions.
- **The Record's shelf reads forward** *(2026-08-11)* — the eight in the order they next come round, each line saying when. It was the eight in authored order against eight identical "unkept"s, which is a checklist of things already missed rather than a calendar, and it left no surface anywhere naming a date.
