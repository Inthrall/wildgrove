...
| Sidekick overlay (**due July 2026**)                          | No code: release with "Sidekick is on by default", then Testing → Advanced settings → *automatically* on for new bundles — required, or each CI upload lands without it. SDK route is APK-only; minSdk 26 already clears its 23 | 5     |
