...
- **A site is watched by the body standing at it (DECIDED 2026-08-12).** Each site is its own post (§2), and only that post's holder sketches there: an unwatched site records nothing, trains nothing, and surfaces no deep amber, and its pity clock **freezes** where it stood rather than resetting (those hours were watched, and moving one companion around the map must not be worse than leaving them still). The watch card on the Trail page is where each site's watcher is sent, and it now sits inside that zone's own section.
